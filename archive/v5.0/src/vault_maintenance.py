"""Vault dedupe, delete cascade, index rebuild, untagged/face cleanup."""
...
def dedupe_vault(config: Dict[str, Any], *, quiet: bool = False) -> Dict[str, int]:
    """
    Remove duplicate vault JSON that resolve to the same media file on disk.
  Keeps the best entry per resolved path (see metadata.dedupe_prefer).
    """
    store = MetadataStore(config)
    ...
    for jp in iter_vault_json_paths(store):
        ...
        if resolved is not None:
            ...
            groups.setdefault(key, []).append((jp, doc, resolved))
        else:
            md5 = _doc_md5(doc)
            if md5:
    ...
    for _key, items in groups.items():
        ...
        # Rewrite keeper at canonical path hash if needed
        if keeper_res is not None:
            try:
                ...
                keeper_doc["updated_at"] = datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%SZ")
...
def delete_media_cascade(
    ...
    *,
    ...
    """
    targets: list of dicts with optional keys full_path, metadata_json_path, media_id
    """
    store = MetadataStore(config)
    stats = {
        "files_deleted": 0,
    ...
    for t in targets:
        ...
        if person_id and remove_untagged_folder(config, person_id):
