...
# ============================================================
"""Excel Report v2.1 - metadata_status, date_source, hyphen naming."""
...
class ExcelWriter:
    ...
    def write(self, records, scan_folder, analytics_data=None):
        ...
        try:
            if self.do_sum:
                print("  Writing Summary..."); self._summary(wb, records, scan_folder); wb.save(op); print("  ✓ Summary")
            if self.do_all:
                print("  Writing All Images..."); self._all(wb, records); wb.save(op); print("  ✓ All Images")
            if self.do_blur:
                print("  Writing Blurry..."); self._blurry(wb, records); wb.save(op); print("  ✓ Blurry")
            if self.do_dup:
                print("  Writing Duplicates..."); self._dups(wb, records); wb.save(op); print("  ✓ Duplicates")
            if self.do_qual:
                print("  Writing Quality..."); self._quality(wb, records); wb.save(op); print("  ✓ Quality")
            if self.do_analytics and analytics_data:
                print("  Writing Analytics..."); self._analytics_sheet(wb, analytics_data); wb.save(op); print("  ✓ Analytics")
            if self.do_clusters:
                clustered = [r for r in records if r.get('cluster_label')]
                if clustered:
                    print("  Writing Clusters..."); self._clusters_sheet(wb, records); wb.save(op); print("  ✓ Clusters")
            return str(op)
    ...
    def _quality(self, wb, records):
        ...
        for label, lo, hi in [('Excellent', 80, 101), ('Good', 60, 80), ('Fair', 40, 60), ('Poor', 0, 40)]:
            ...
            rows.append((f'{label} ({lo}-{min(hi-1, 100)}%)', n, f"{n/tq*100:.1f}%"))

        # Metadata status
        ms_counts = Counter(r.get('metadata_status', 'Unknown') for r in records)
        rows.extend([('', '', ''), ('METADATA STATUS', 'Count', '')])
        for status, cnt in ms_counts.most_common():
            rows.append((status, cnt, ''))

        # Date source
        ds_counts = Counter(r.get('date_source', 'None') for r in records)
        rows.extend([('', '', ''), ('DATE SOURCE', 'Count', '')])
        for src, cnt in ds_counts.most_common():
            rows.append((src, cnt, ''))
        ...
        if vid_records:
            vid_dur = [r for r in vid_records
                       if isinstance(r.get('video_duration_sec'), (int, float))
                       and r.get('video_duration_sec', 0) > 0]
            from .utils import format_duration
            total_dur = sum(r['video_duration_sec'] for r in vid_dur) if vid_dur else 0
            rows.extend([
                ('', '', ''),
                ('VIDEO STATS', 'Count', ''),
                ('Total Videos', len(vid_records), ''),
                ('With Duration', len(vid_dur), ''),
        ...
        secs = {'QUALITY STATS', 'DISTRIBUTION', 'METADATA STATUS', 'DATE SOURCE',
                'FACE DETECTION', 'VIDEO STATS', 'VIDEO CODECS'}
    ...
    def _summary(self, wb, records, sf):
        ...
        partial_exif = sum(1 for r in records if r.get('metadata_status') == 'Partial EXIF')
        no_exif = sum(1 for r in records if r.get('metadata_status') == 'No EXIF')
        exif_date = sum(1 for r in records if r.get('date_source') == 'EXIF')
        file_date = sum(1 for r in records if r.get('date_source') == 'File Modified')

        from .utils import format_duration
        vd = [r.get('video_duration_sec') for r in records
        ...
        rows = [
            ...
            ('METADATA QUALITY', ''),
            ('Full EXIF', full_exif),
            ('Partial EXIF', partial_exif),
            ('No EXIF', no_exif),
            ('Date from EXIF', exif_date),
            ('Date from File', file_date),
            ('', ''),
            ('VIDEO', ''),
            ('Total Duration', tvd),
            ('Videos with Duration', len(vd)),
            ('', ''),
            ('QUALITY', ''),
            ('Blurry', bc),
            ('Duplicates', dc),
        ...
        secs = {'GENERAL', 'METADATA QUALITY', 'VIDEO', 'QUALITY', 'ENRICHMENT', 'BY FORMAT'}
        hf = PatternFill('solid', fgColor=self.C_SEC)
        acf = PatternFill('solid', fgColor=self.C_ACC)
