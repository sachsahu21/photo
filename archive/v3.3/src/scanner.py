        ...
        if ft == 'image':
            ...
            if pi:
                ...
                try:
                    ...
                    if vm:
                        for vk, vv in vm.items():
                            if vv is not None:
                                d[vk] = vv
                        if vm.get('video_width'):
                            d['width'] = vm['video_width']
                        if vm.get('video_height'):
                            d['height'] = vm['video_height']
                    logger.debug('Video %s: dur=%s fmt=%s w=%s h=%s',
                                fp.name,
                                d.get('video_duration_sec'),
                                d.get('video_duration_fmt'),
                                d.get('video_width'),
                                d.get('video_height'))
                except Exception as e:
                    d['error'] = str(e)[:120]
                    logger.warning('Video extract error %s: %s', fp.name, e)
