...
# ============================================================
"""Image Scanner - Main Entry Point (Enhanced)"""
...
def setup_logging(config):
    ...
    lf = config.get('logging.file', './logs/image_scanner.log')
    ...
    logging.basicConfig(level=getattr(logging, str(lv).upper(), logging.INFO),
...
def load_backup(fn=BACKUP_FILE):
    try:
        if not Path(fn).exists():
            print(f"  ✗ Not found: {fn}"); return None
        with open(fn, 'rb') as f:
            r = pickle.load(f)
        print(f"  ✓ Loaded {len(r)} records"); return r
    except Exception as e:
...
def print_banner():
    print("""
    ╔═══════════════════════════════════════════════════════╗
    ║      IMAGE SCANNER - Professional Edition v2.0        ║
...
def task_1(config, logger):
    ...
    try:
        ...
        if fc: print(f"  ✓ {fc} faces detected")
        if gc: print(f"  ✓ {gc} geolocated")
        if tc: print(f"  ✓ {tc} auto-tagged")
        if cc: print(f"  ✓ {cc} clusters")
        ...
        if config.get('comparison.enabled', True):
            try:
                ...
                if pages:
                    print(f"  ✓ {len(pages)} comparison pages generated")
            except Exception as e:
        ...
        box("TASK 1 COMPLETE", [
            ("Files", len(records)), ("Blurry", bc), ("Duplicates", dc),
            ("Groups", dg), ("Faces", fc), ("Geolocated", gc),
            ("Tagged", tc), ("Clusters", cc), ("Excel", ep)])
        return ep

    except Exception as e:
        logger.error(f"Task 1: {e}", exc_info=True)
        print(f"\n  ✗ {e}")
        if records: save_backup(records)
        return None
...
def task_1b(config, logger):
    ...
    try:
        ...
        logger.error(f"Task 1B: {e}", exc_info=True); print(f"  ✗ {e}"); return None
...
def task_2(excel_path, logger):
    ...
    try:
        ...
        if not sn: print("  ✗ No sheet"); return
        ws = wb[sn]
        dc = fc = nc = None
        for ci, c in enumerate(ws[1], 1):
            if not c.value: continue
            h = str(c.value).strip().lower()
            if 'delete' in h: dc = ci
            elif h == 'full path': fc = ci
            elif h == 'filename': nc = ci
        if not dc or not fc: print("  ✗ Columns not found"); return

        to_del = []
        for ri in range(2, ws.max_row + 1):
            v = ws.cell(row=ri, column=dc).value
            if v and str(v).strip().lower() in ('yes','true','1'):
                to_del.append((ws.cell(row=ri, column=fc).value, ws.cell(row=ri, column=nc).value if nc else '?'))

        if not to_del: print("  ✓ Nothing to delete"); return
        print(f"  {len(to_del)} files marked.")
        if input("  Confirm? (yes/no): ").strip().lower() != 'yes': print("  Cancelled"); return

        ok = err = nf = 0
        for fp, fn in to_del:
            if not fp: err += 1; continue
            try:
                p = Path(fp)
                if p.exists(): p.unlink(); ok += 1; print(f"  ✓ {fn}")
                else: nf += 1
            except Exception: err += 1
        box("TASK 2 DONE", [("Deleted", ok), ("Not found", nf), ("Errors", err)])
    except Exception as e:
        logger.error(f"Task 2: {e}", exc_info=True); print(f"  ✗ {e}")
...
def task_3(excel_path, config, logger):
    ...
    try:
        ...
        h2k = {'Filename':'filename','Folder':'folder','Full Path':'full_path','Date Taken':'date_taken',
               'File Modified':'file_modified','DELETE? (Yes/No)':'delete_flag','Duplicate?':'is_duplicate',
        ...
        box("TASK 3 DONE", [("Organized", s), ("Errors", e), ("Output", config.get('organization.output_folder',''))])
    except Exception as e:
        logger.error(f"Task 3: {e}", exc_info=True); print(f"  ✗ {e}")
...
def task_6_comparisons(config, logger):
    """Generate comparison pages from backup."""
    ...
    try:
        ...
        cg = ComparisonGenerator(config.get('comparison.output_folder', './comparisons'))
        pages = cg.generate(records)
        print(f"  ✓ {len(pages)} comparison pages in {config.get('comparison.output_folder', './comparisons')}")
    except Exception as e:
        logger.error(f"Comparison: {e}"); print(f"  ✗ {e}")
...
def main():
    try:
        ...
        logger.info("Image Scanner v2.0 started")
        print(f"  Config: {config.config_path}")
        print(f"  Scan: {config.get('scan.folder_path')}")

        features = []
        if config.get('face_detection.enabled'): features.append('faces')
        if config.get('auto_tagging.enabled'): features.append('tags')
        if config.get('geocoding.enabled'): features.append('geo')
        if config.get('clustering.enabled'): features.append('clusters')
        if config.get('thumbnails.enabled'): features.append('thumbs')
        if features:
            print(f"  Features: {', '.join(features)}")

        if not config.validate():
            if input("  Continue? (yes/no): ").strip().lower() != 'yes': return

        last = None
        while True:
            print_menu()
            ch = input("  Choice: ").strip().lower()
            if ch == '1': last = task_1(config, logger)
            elif ch == '1b': last = task_1b(config, logger)
            elif ch == '2':
                p = input("  Excel (Enter=last): ").strip() or last
                if p and Path(p).exists(): task_2(p, logger)
                else: print(f"  ✗ Not found: {p}")
            elif ch == '3':
                p = input("  Excel (Enter=last): ").strip() or last
                if p and Path(p).exists(): task_3(p, config, logger)
                else: print(f"  ✗ Not found: {p}")
            elif ch == '4':
                last = task_1(config, logger)
                if last:
                    if input("  Delete? (yes/no): ").strip().lower() == 'yes': task_2(last, logger)
                    if input("  Organize? (yes/no): ").strip().lower() == 'yes': task_3(last, config, logger)
            elif ch == '5': task_5_streamlit()
            elif ch == '6': task_6_comparisons(config, logger)
            elif ch == '0': print("\n  ✓ Goodbye!"); break
            else: print("  ✗ Invalid")
            input("\n  Enter to continue...")
        ...
        print(f"\n  ✗ Fatal: {e}"); logging.error(f"Fatal: {e}", exc_info=True); sys.exit(1)
