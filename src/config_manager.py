...
class ConfigManager:
    ...
    def _deep_merge(base, override):
        """Merge override into base recursively; base provides defaults for missing keys."""
        result = dict(base)
        ...
        if not self.config_path.exists():
            ...
            try:
                ...
                if isinstance(loaded, dict):
                    self.config = self._deep_merge(self._defaults(), loaded)
                else:
                    self.config = self._defaults()
            except Exception:
                self.config = self._defaults()
    ...
    def _defaults(self):
        return {
            ...
            'enrich': {
                'target_fields': [
                    'date_taken', 'camera_model', 'blur_score',
