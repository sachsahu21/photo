...
class ImageScanner:
    ...
    def scan_files(self, file_paths, batch_callback=None, fields_needed=None):
        """Scan a specific list of files.

        fields_needed: optional set of field names that must be populated.
          When provided, _extract skips expensive blocks (hashing, EXIF, blur, faces)
          that produce none of the requested fields, saving significant time during enrich.
        """
        self._init_features()
        files = []
        for fp in file_paths or []:
            ...
            if p.is_file() and p.suffix.lower() in self.all_exts:
                files.append(p)
        files = sorted(files)
        # use parallel when threads > 1 and enough files; wrap batch_callback with a lock so
        # concurrent workers can call it safely without corrupting the pending list
        if self.parallel_workers != 1 and len(files) > 50 and not self.fast_mode:
            import threading
            _cb_lock = threading.Lock()

            def _safe_callback(rec):
                if batch_callback is not None:
                    with _cb_lock:
                        batch_callback(rec)

            return self._scan_parallel_with_callback(files, _safe_callback, fields_needed=fields_needed)
        return self._scan_sequential(files, None, batch_callback=batch_callback, fields_needed=fields_needed)
    ...
    def _scan_sequential(self, files, checkpoint, batch_callback=None, fields_needed=None):
        records = []
        it = tqdm(files, desc='Scanning', unit='file', disable=not self.show_progress)
        for fp in it:
            try:
                rec = self._finalize_record(self._extract(fp, fields_needed=fields_needed))
                records.append(rec)
                if checkpoint:
                    checkpoint.mark_processed(str(fp))
                if batch_callback:
    ...
    def _scan_parallel_with_callback(self, files, batch_callback, fields_needed=None):
        """Parallel scan that fires batch_callback (already lock-wrapped) after each record."""
        from .parallel_processor import ParallelProcessor
        workers = self.parallel_workers if self.parallel_workers > 0 else 4
        pp = ParallelProcessor(max_workers=workers, show_progress=self.show_progress)
        ...
        results = pp.process(files, process_one, desc='Enriching (parallel)')
        return [r for r in results if r is not None]
    ...
    # Fields produced by each expensive block — used by fields_needed gate
    _HASH_FIELDS = frozenset({'md5_hash', 'sha256_hash'})
    _EXIF_FIELDS = frozenset({
        'width', 'height', 'megapixels', 'aspect_ratio', 'orientation',
        'date_taken', 'camera_make', 'camera_model', 'focal_length',
        'aperture', 'shutter_speed', 'iso', 'flash', 'gps_lat', 'gps_lon',
        'gps_altitude', 'has_exif', 'color_space',
    })
    _BLUR_FIELDS = frozenset({'blur_score', 'is_blurry', 'quality_rating', 'quality_score', 'quality_issues'})
    _FACE_FIELDS = frozenset({'face_count', 'face_category', 'face_search_ready', 'face_boxes'})

    def _extract(self, filepath, fields_needed=None):
        """Extract metadata for a single file.

        fields_needed: if set, only run expensive blocks (hash/EXIF/blur/face) that
          produce at least one field in the set.  Blocks with no overlap are skipped,
          making targeted enrich runs significantly faster.
        """
        fn = fields_needed  # short alias; None means run everything
        ...
        md5_hash = ''
        sha256_hash = ''
        need_hash = fn is None or bool(fn & self._HASH_FIELDS)
        if need_hash and not (file_type == 'video' and self.skip_video_hash):
        ...
        if file_type == 'image':
            ...
            need_exif = fn is None or bool(fn & self._EXIF_FIELDS)
            need_blur = self.blur_enabled and (fn is None or bool(fn & self._BLUR_FIELDS))
            need_face = self.face_enabled and (fn is None or bool(fn & self._FACE_FIELDS))

            if PIL_OK and need_exif:
                try:
                    pil_img = PILImage.open(filepath)
                except Exception as e:
                    defaults['error'] = str(e)[:120]

            if CV2_OK and (need_blur or need_face):
                try:
            ...
            if need_blur and cv2_img is not None:
                try:
            ...
            if need_face and self._face_detector:
                bgr = cv2_img
                if bgr is None:
