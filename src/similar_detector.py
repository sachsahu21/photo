...
class SimilarDetector:
    ...
    def compute_hashes(self, records):
        ...
        for i, rec in enumerate(records):
            ...
            try:
                ...
                # Convert to grayscale once; reuse for ahash, phash, and dhash.
                gray_img = img.convert('L')
                pixels = np.array(gray_img.resize((hs, hs), PILImage.LANCZOS), dtype=float)
                ...
                if self.dhash_on:
                    dhash_pixels = np.array(gray_img.resize((hs + 1, hs), PILImage.LANCZOS), dtype=float)
                ...
                if self.sift_on and CV2_OK:
                    # Pass the already-loaded grayscale array to avoid re-reading from disk.
                    gray_cv = np.array(gray_img)
                    rec['sift_desc'] = self._compute_sift(fp, gray_cv=gray_cv)
    ...
    def _compute_sift(self, filepath, gray_cv=None):
        try:
            img = gray_cv if gray_cv is not None else cv2.imread(str(filepath), cv2.IMREAD_GRAYSCALE)
            if img is None:
                return None
    ...
    def _bits_to_int(self, bits):
        arr = np.packbits(np.asarray(bits, dtype=np.uint8))
        return int.from_bytes(arr.tobytes(), "big")
    ...
    def get_stats(self, records):
        ...
        hashed = sum(
            1 for r in records
            if r.get('file_type') == 'image'
            and any(r.get(k) is not None for k in ('ahash', 'phash', 'dhash', 'color_hist', 'sift_desc'))
        )
        similar = sum(1 for r in records if str(r.get('is_similar', '')).upper() == 'YES')
        groups = len(set(
            r.get('similar_group', '')
            for r in records
            if r.get('similar_group') and str(r.get('similar_group', '')).strip()
        ))
