...
class ExcelWriter:
    ...
    def _all(self, wb, records):
        ...
        ncols = len(self.ALL_COLS)

        for ri, rec in enumerate(records, 2):
            # ws.append is ~5-10x faster than per-cell ws.cell() writes
            ws.append([self._sv(rec.get(k, '')) for k, _, _ in self.ALL_COLS])
            # Apply fill only to special rows (dups/blurry) — skip alternating fill
            is_blurry = rec.get('is_blurry') is True
            is_dup = str(rec.get('is_duplicate', '')).upper() == 'YES'
            if is_blurry or is_dup:
                fl = bf if is_blurry else df
                for ci in range(1, ncols + 1):
                    ws.cell(row=ri, column=ci).fill = fl

        ws.auto_filter.ref = f"A1:{get_column_letter(ncols)}{len(records) + 1}"
    ...
    def _dups(self, wb, records):
        ...
        for ri, rec in enumerate(dups, 2):
            ...
            ws.append([self._sv(rec.get(k, '')) for k, _, _ in self.DUP_COLS])
            # Apply group color + bold for best — only iterate cells for styled rows
            row_font = bfont if ib else nfont
            for ci in range(1, ncols + 1):
                ...
                if ib:
                    cell.font = row_font
        if dups:
            ws.auto_filter.ref = f"A1:{get_column_letter(ncols)}{len(dups) + 1}"
