...
| **22** | `Execute quarantine actions from Excel` | Reads the Excel workbook, finds rows marked for deletion, moves the corresponding files and vault entries to quarantine, then reconciles the vault. | [task_2](file:///C:/Users/ISSUser/Desktop/Sachin/git/photo/v5.2/main.py#L706-L712) |
...
| `task_2` | Excel-driven quarantine - parses the workbook, collects rows with a *Delete* flag, moves files and JSON metadata to quarantine, then runs a dedupe + reconcile. | `openpyxl`, `src.vault_maintenance.quarantine_media_cascade` |
