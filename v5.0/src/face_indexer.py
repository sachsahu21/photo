    ...
    def find_person(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for person_label, person_dir in person_dirs:
                seed_vec = self._embed_seed_person(person_dir)
                if seed_vec is None:
                    continue

                sims = _cosine_sim_matrix(seed_vec, mat)
                per_file_best: Dict[str, float] = {}
                for p, s in zip(paths, sims.tolist()):
                    if s >= self.sim_thr:
                        if p not in per_file_best or s > per_file_best[p]:
                            per_file_best[p] = float(s)
