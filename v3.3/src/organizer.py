"""Image Organizer v2.6 - flat / year / year-month-date with mixed converter and date merge"""
...
class ImageOrganizer:
    ...
    def _org_ss(self, ss):
        ...
        for mk, grp in mg.items():
            ...
            for rec, dt in tqdm(grp, desc='  ' + fn, disable=not self.show):
                ...
                if self.structure == 'year' and sdt:
                    dd = self.output / str(sdt.year) / fn
                elif self.structure == 'year-month-date' and sdt:
    ...
    def convert_structure(source_folder, target_structure, target_folder=None):
        ...
        if not folders:
            ...
            return False
        flat_count = sum(1 for f in folders if f['current'] == 'flat')
        year_count = sum(1 for f in folders if f['current'] == 'year')
        ymd_count = sum(1 for f in folders if f['current'] == 'year-month-date')
        misc_count = sum(1 for f in folders if f['current'] == 'misc')
        print('  Found ' + str(len(folders)) + ' folders:')
        print('    flat:            ' + str(flat_count))
        print('    year:            ' + str(year_count))
        print('    year-month-date: ' + str(ymd_count))
        print('    misc/undated:    ' + str(misc_count))
        need_move = sum(1 for f in folders if f['current'] != target_structure and f['current'] != 'misc')
        already = sum(1 for f in folders if f['current'] == target_structure)
        print('  Already in target: ' + str(already))
        print('  Need conversion:   ' + str(need_move))
        ...
        for fd in tqdm(folders, desc='  Converting', disable=False):
                ...
                if target_folder and str(dest) != str(source):
                    ...
                    if str(new_path) != str(old_path):
                        try:
                            ...
                            errors += 1
                            logger.error('Convert misc %s: %s', fd['path'], e)
                continue
            ...
            try:
                ...
                errors += 1
                logger.error('Convert error %s -> %s: %s', old_path, new_path, e)
        ...
        print('')
        print('  Merging same-date folders...')
        merge_count = _merge_same_date_folders(dest, target_structure)
        if merge_count > 0:
    ...
    for dk, group_folders in sorted(date_groups.items()):
        ...
        for folder in group_folders:
            ...
            if text and not best_text:
                ...
                best_folder = folder
                best_count = count
        print('    Merging ' + str(len(group_folders)) + ' folders for date ' + dk + ':')
        for folder in group_folders:
            fc = _count_files(folder)
            txt = _extract_text_suffix(folder.name)
            marker = ' [KEEP]' if folder == best_folder else ' [MERGE]'
            print('      ' + folder.name + ' (' + str(fc) + ' files' + (', text: ' + txt if txt else '') + ')' + marker)
        for folder in group_folders:
            ...
            try:
                ...
                logger.error('Merge error %s -> %s: %s', folder, best_folder, e)
        total = _count_files(best_folder)
        if best_text:
            new_name = dk + '-' + format_pic_count(total) + '-' + best_text
        else:
            new_name = dk + '-' + format_pic_count(total)
        if new_name != best_folder.name:
            new_path = best_folder.parent / new_name
            if new_path.exists() and str(new_path) != str(best_folder):
                logger.warning('Cannot rename %s -> %s: exists', best_folder.name, new_name)
            else:
                try:
                    best_folder.rename(new_path)
                    print('      Result: ' + new_name + ' (' + str(total) + ' files)')
                except Exception as e:
                    logger.warning('Rename failed: %s', e)
        else:
            print('      Result: ' + best_folder.name + ' (' + str(total) + ' files)')
        merge_count += 1
    return merge_count
...
def _update_pic_count(folder_path, new_count):
    ...
    if new_path.exists() and str(new_path) != str(folder_path):
        logger.warning('Cannot update count %s -> %s: target exists', old_name, new_name)
        return None
    if new_name == old_name:
        return None
    try:
        folder_path.rename(new_path)
        print('    Count update: ' + old_name + ' -> ' + new_name)
        return new_name
    except Exception as e:
        logger.warning('Count update failed %s: %s', old_name, e)
        return None
...
def _extract_folder_info(item):
    ...
    if ss_match and RE_SS_FOLDER.search(name):
        return {
            'path': str(item), 'name': name,
            'year': ss_match.group(1), 'month': ss_match.group(2),
        }
