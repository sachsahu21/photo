...
def _clear_dup_flags(r):
    """Reset duplicate fields on a single record without touching user-set delete_flag."""
    r['is_duplicate'] = 'No'
    r['duplicate_group'] = ''
    r['duplicate_type'] = ''
    r['master_media_id'] = ''
...
def _recompute_duplicate_metadata(config, new_hashes=None, all_records=None):
    ...
    from duplicate detection — they must not cause their surviving siblings to
    remain falsely flagged as duplicates.  Their own flags are cleared so Excel
    doesn't show missing files as duplicates either.

    When new_hashes is provided (set of hash strings from the current scan batch),
    ...
    # Partition: only files that exist on disk participate in dup detection.
    # An orphaned record for a deleted/quarantined file must not inflate groups.
    present, orphaned = [], []
    for r in all_records:
        fp = str(r.get('full_path') or '').strip()
        if fp and Path(fp).is_file():
            present.append(r)
        else:
            orphaned.append(r)

    # Clear stale dup flags on orphaned records.
    for r in orphaned:
        _clear_dup_flags(r)

    if not new_hashes:
        # Full recompute on present files only.
        updated = DuplicateHandler(
    ...
    existing_dup_hashes = {
        str(r.get(hash_field) or '').strip().lower()
        for r in present
        if r.get('is_duplicate') or r.get('duplicate_group')
    }
    affected_hashes = (new_hashes | existing_dup_hashes) - {'', 'none'}

    affected = [r for r in present if str(r.get(hash_field) or '').strip().lower() in affected_hashes]
...
def _handle_missing_metadata(missing_paths, config, logger):
    ...
    if not sys.stdin.isatty():
        print('  Non-interactive mode — skipping missing metadata prompt (run option 21 to repair).')
        return
    while True:
...
def task_analyze_folders(config, logger):
    ...
    if not sys.stdin.isatty():
        p = default_path
        print('  Non-interactive mode — using scan.folder_path: ' + str(p))
    else:
...
def task_reconcile_paths(config, logger):
    ...
    try:
        ...
        if config.get('duplicates.enabled', True):
            ...
            print('  Duplicate flags refreshed.')
    except Exception as e:
...
def task_enrich_metadata(config, logger):
    ...
    try:
        ...
        _ENRICH_FIELDS = tuple(enrich_cfg.get('target_fields') or
                               ['date_taken', 'camera_model', 'blur_score', 'width', 'height', 'gps_lat', 'gps_lon'])
        ...
        enrich_cfg = config.get('enrich') or {}
        defer_index_rebuild = bool(enrich_cfg.get('defer_index_rebuild', True))
        try:
            interval = int(
                enrich_cfg.get('flush_interval') or
                config.get('processing.metadata_flush_interval', 200) or 200
            )
        except (TypeError, ValueError):
            interval = 200
        interval = max(1, interval)
        ...
        def flush(force=False):
            ...
            batch = _with_scan_defaults(list(pending))
            md_store.upsert_records(batch)
            # checkpoint saved after successful upsert (atomic: crash before upsert won't mark files done)
        ...
        scanner_cfg = config.to_dict()
        # honour enrich.use_parallel — if false, force single-thread for enrich
        if not bool(enrich_cfg.get('use_parallel', True)):
            scanner_cfg.setdefault('processing', {})['threads'] = 1
        scanner = ImageScanner(scanner_cfg)
        # pass the union of all missing fields so _extract skips blocks not needed
        ...
        scanner.scan_files(files, batch_callback=on_record,
                           fields_needed=all_missing_fields if all_missing_fields else None)
        flush(force=True)
        if enriched:
