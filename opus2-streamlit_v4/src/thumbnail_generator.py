...
class ThumbnailGenerator:
    def __init__(self, output_folder='./thumbnails', size=None):
        ...
        self.size = tuple(size) if size else (150, 100)
    ...
    def generate_for_video(self, filepath):
        ...
        try:
            fp = Path(filepath)
            tp = self.output_folder / ('vthumb-' + fp.stem + '.jpg')
            if tp.exists():
                return str(tp)
            ...
            cv2.imwrite(str(tp), cv2.resize(frame, (int(w * s), int(h * s))))
            return str(tp)
        except Exception:
            return None
